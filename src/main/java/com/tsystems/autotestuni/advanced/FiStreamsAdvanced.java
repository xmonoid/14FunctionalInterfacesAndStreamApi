...
public class FiStreamsAdvanced {
    ...
    public static Optional<Book> cheapestBook(Stream<Book> bookStream, int yearFrom, int yearTo) {
        return bookStream
                .filter(book -> book.getPublishYear() >= yearFrom && book.getPublishYear() < yearTo)
                .min((one, two) -> {
            double difference = one.getCurrentPrice() - two.getCurrentPrice();
            if (difference < 0) {
                return -1;
    ...
    public static double totalPriceOfBooks(Stream<Book> bookStream, char firstLetter) {
        return bookStream
                .filter( book -> book.getAuthor().charAt(0) == firstLetter )
                .mapToDouble( Book::getCurrentPrice )
                .sum();
    }
}
